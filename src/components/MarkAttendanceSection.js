...
export default function MarkAttendanceSection({
  ...
  onStudentDetailView // Function to open modal
}) {
  ...
  const weekDay = 'Week 1 - Tuesday - 21 Jan 2020'; // Example
        ...
        <table className="min-w-full bg-white divide-y divide-gray-100">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Marks</th> {/* For student detail pop-up */}
            ...
            {students.map((student, i) => (
              <tr key={student.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <div className="flex justify-center items-center gap-1">
                    <button
                      ...
                      className={`p-2 rounded-full transition-colors ${
                        student.status === 'present' ? 'bg-green-100 text-green-600' : 'text-gray-400 hover:bg-green-50'
                      }`}
                      title="Present"
                    >
                      <CheckCircle size={18} />
                    ...
                    <button
                      ...
                      className={`p-2 rounded-full transition-colors ${
                        student.status === 'absent' ? 'bg-red-100 text-red-600' : 'text-gray-400 hover:bg-red-50'
                      }`}
                      title="Absent"
                    >
                      <XCircle size={18} />
                    ...
                    <button
                      ...
                      className={`p-2 rounded-full transition-colors ${
                        student.status === 'late' ? 'bg-yellow-100 text-yellow-600' : 'text-gray-400 hover:bg-yellow-50'
                      }`}
                      title="Late"
                    >
                      <MinusCircle size={18} />
                ...
                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                  ...
                  >
                    <Pencil size={16} /> {/* Or Eye icon for view */}
                  </button>
